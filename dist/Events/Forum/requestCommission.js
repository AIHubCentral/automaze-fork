...
module.exports = {
    ...
    async run(client, Threader, newlyCreated) {
        try {
            ...
            if (!client.botConfigs.commissions.sendMessages)
                return;
            ...
            if (Threader.guildId != client.discordIDs.Guild)
                return;
            if (Threader.parentId != client.discordIDs.Forum.RequestModel.ID)
                return;
            ...
            if (client.botConfigs.logs.modelRequests) {
                client.logger.debug('New model request', {
                    more: {
                        guildId: Threader.guild.id,
                    }
                });
            }
        }
