import {
	...
	ActionRowBuilder, ComponentType, User, ChatInputCommandInteraction, EmbedBuilder,
	InteractionReplyOptions, InteractionResponse, InteractionUpdateOptions, ButtonBuilder, ButtonStyle
} from 'discord.js';
...
const Guides: SlashCommand = {
	...
	async execute(interaction) {
		...
		if (category === 'realtime') {
			...
			if (!guideForRealtime) return await interaction.reply({ content: 'This guide is not available in the selected language.', ephemeral: true });
		...
		if (!selectedGuide) return interaction.reply({ content: "This guide is not available in the selected language.", ephemeral: true });

		const botResponse: InteractionReplyOptions = prepareGuideReply(selectedGuide, targetUser, botConfigs.colors);
...
function createSelectMenu(content: LanguageData): ActionRowBuilder<StringSelectMenuBuilder> {
	...
	const selectMenuActionRow = new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(realtimeGuidesSelectMenu);

	return selectMenuActionRow;
}

async function handleRealtimeGuide(content: LanguageData, mainUser: User, targetUser: User | null, interaction: ChatInputCommandInteraction): Promise<void> {
...
function prepareGuideReply(guide: LanguageData, targetUser: User | null, embedColors: IConfigColors): InteractionReplyOptions {
